...
def symmetrize_distance(df, from_col, to_col, dist_col):
    """Make distance dict symmetric."""
    d = {}
    for _, r in df.iterrows():
        a, b, dist = str(r[from_col]), str(r[to_col]), r[dist_col]
        if pd.notna(a) and pd.notna(b) and pd.notna(dist):
            d[(a, b)] = float(dist)
            d[(b, a)] = float(dist)
    return d
...
def run_vrp_with_prep(df_st, df_routes, df_alloc):
    ...
    # Fallback distance (avg of known)
        ...
        for j, b in enumerate(node_list):
            if a == b:
                ...
                if k in dur_dict:
                    ...
                    distance_matrix[i, j] = int(round(dist_dict[k] * 2))  # min estimate
                else:
                    distance_matrix[i, j] = int(round(avg_dist * 2))  # fallback
...
# STREAMLIT UI
# =============================
st.title("🚛 CNG LCV Scheduling Optimization with 45-min Prep Time (Fixed Distance & Load)")
